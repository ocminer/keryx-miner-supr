...
// mode. Do not add direct fprintf/fputs calls to this wrapper.
extern "C" void ggml_log_internal(enum ggml_log_level level, const char* format, ...);
#define KERYX_LOG_INFO(...)  ggml_log_internal(GGML_LOG_LEVEL_INFO, __VA_ARGS__)
#define KERYX_LOG_WARN(...)  ggml_log_internal(GGML_LOG_LEVEL_WARN, __VA_ARGS__)
#define KERYX_LOG_ERROR(...) ggml_log_internal(GGML_LOG_LEVEL_ERROR, __VA_ARGS__)
...
static bool walk_init(KeryxLlama* h) {
    ...
    if (!g) {
        KERYX_LOG_ERROR("keryx-llama-vk: gguf meta parse failed\n");
        return false;
    }
    ...
    if (!model_dev) {
        KERYX_LOG_ERROR("keryx-llama-vk: no VK-resident tensor found (model on CPU?)\n");
    ...
    if (!located) {
        KERYX_LOG_ERROR("keryx-llama-vk: could not match the model's VkDevice (no BDA?)\n");
        gguf_free(g); if (meta) ggml_free(meta);
        return false;
    }
...
// Optional presentation ABI; see the CUDA/Metal wrapper for the host-side contract.
...
KeryxLlama* keryx_llama_load(const char* gguf_path, int gpu, int n_ctx) {
    ...
    if (gpu < 0) {
        ...
        if (d < 0 && std::getenv("KERYX_LLAMA_VK_AUTO_PCI_ALLOWLIST") != nullptr) {
            KERYX_LOG_ERROR("keryx-llama-vk: no Vulkan device matches the selected OpenCL worker PCI allowlist\n");
            return nullptr;
        }
        gpu = d >= 0 ? d : 0;
    ...
    if (!walk_init(h)) {
        ...
        KERYX_LOG_WARN("keryx-llama-vk: walk init failed — inference-only mode\n");
    }
    return h;
}
