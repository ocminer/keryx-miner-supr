...
# A cached /tmp checkout is convenient, but it must not make the release non-reproducible. After
# removing our generated append, require the exact pinned tag and an otherwise pristine llama.cpp
# tree; delete the cache yourself if either check fails.
if [[ "$(git -C "$SRC" describe --tags --exact-match 2>/dev/null || true)" != "$TAG" ]] \
   || [[ -n "$(git -C "$SRC" status --porcelain=v1 --untracked-files=all --ignore-submodules)" ]]; then
  echo "ERROR: $SRC is not a clean checkout of llama.cpp $TAG; remove it and rebuild" >&2
  exit 1
fi
...
docker run --rm \
  ...
  llama-vk-toolchain bash -euo pipefail -c '
    ...
    g++ -O2 -std=c++17 -shared -fPIC -fopenmp /repo/tools/keryx-llama/keryx_llama_vk.cpp \
      ...
      -Wl,--end-group \
      -Wl,-rpath,\$ORIGIN -lvulkan -lpthread -ldl -o /out/$OUTNAME
  ...
  '
MAX_GLIBC=$(objdump -T "$OUT/$OUTNAME" 2>/dev/null | grep -oE 'GLIBC_[0-9.]+' | sort -V | tail -1 || true)
[[ -n "$MAX_GLIBC" ]] || { echo "ERROR: could not determine glibc floor for $OUTNAME" >&2; exit 1; }
if [[ "$(printf '%s\n' GLIBC_2.31 "$MAX_GLIBC" | sort -V | tail -1)" != "GLIBC_2.31" ]]; then
  echo "ERROR: $OUTNAME requires $MAX_GLIBC, above the promised HiveOS GLIBC_2.31 ceiling" >&2
  exit 1
fi
echo ">> $OUTNAME: $(ls -la "$OUT/$OUTNAME" | awk '{print $5}') bytes, glibc=$MAX_GLIBC, syms=$(nm -D "$OUT/$OUTNAME" | grep -c keryx_llama)"
...
# Bind the packaged engine to the exact wrapper, Vulkan picker, shaders and build recipe. The AMD
# packagers reject a missing/stale manifest, preventing an old sidecar from silently bypassing new
# selected-worker PCI scoping or disagreeing with the miner's walk ABI.
# shellcheck source=hiveos/amd-inference-route.sh
source "$REPO/hiveos/amd-inference-route.sh"
keryx_write_amd_engine_manifest "$OUT/$OUTNAME" "$VARIANT"
keryx_validate_amd_engine "$OUT/$OUTNAME"
echo ">> $OUTNAME manifest: $OUT/$OUTNAME.manifest"
