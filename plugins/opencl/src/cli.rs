...
pub struct OpenCLOpt {
    ...
    #[clap(
        long = "opencl-no-amd-binary",
        help = "Disable fetching of precompiled AMD kernel (if exists). [default behaviour as of v0.5.2 — kept for compatibility]"
    )]
    pub opencl_no_amd_binary: bool,
    #[clap(
        long = "opencl-use-amd-binary",
        help = "Deprecated compatibility flag. Archived AMD binaries are no longer loaded; the canonical OpenCL source is always JIT-compiled for correctness."
    )]
