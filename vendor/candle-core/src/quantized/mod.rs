...
impl QTensor {
    ...
    /// Zero-dup accessor for the Metal-resident quantized storage (VENDORED PATCH — the only
    /// change vs. upstream candle-core 0.9.2, gated behind `feature = "metal"` so a non-Metal
    /// build compiles this crate byte-for-byte identically to the registry release).
    ///
    /// Upstream keeps `storage` private and `device_ptr()` bails on the Metal branch, forcing
    /// consumers to blit the weights out of the unified-memory `MTLBuffer`. Exposing the
    /// `&QMetalStorage` (which already has a public `buffer()` accessor) lets the PoM Metal miner
    /// build a bindless pointer table over candle's own resident buffers with no packing/copy.
    #[cfg(feature = "metal")]
    pub fn metal_storage(&self) -> Option<&metal::QMetalStorage> {
        match &self.storage {
            QStorage::Metal(s) => Some(s),
            _ => None,
        }
    }
}
